const Reader = {
    ...
    open: async (url, title, id) => {
        ...
        readerView.innerHTML = Reader._getTemplate(title);
        ...
        try {
            ...
            document.getElementById('reader-content').innerHTML = Reader.converter.makeHtml(text);
            Reader.renderNotes();
            if(id && window.SpacedRepetition) SpacedRepetition.markAsReviewed(id);
        } catch (e) {
            document.getElementById('reader-content').innerHTML = '<p class="text-center py-10 text-gray-500">Fehler beim Laden.</p>';
        }
    },
    ...
    _getTemplate: (title) => `
        <div class="sticky top-0 z-[110] bg-[#1c1c1e] border-b border-white/10 shadow-lg" style="padding-top: max(env(safe-area-inset-top), 20px);">
            <div class="h-14 px-4 flex justify-between items-center">
                ...
                <h2 class="text-sm font-bold truncate px-2 text-white">${title}</h2>
                <button id="reader-edit-btn" onclick="Reader.toggleEdit()" class="w-10 h-10 flex items-center justify-center text-gray-400 rounded-full transition active:bg-white/10"><i class="fas fa-pen-nib text-lg"></i></button>
            </div>
        </div>
