const Player = {
    ...
    load: (url, title, id) => {
        ...
        if(playerEl) {
            ...
            // Kleiner Delay damit die Transition greift (Display: block -> Transition)
    ...
    updateIcon: (isPlaying) => {
        ...
        if(icon) {
            // Angepasst für bessere Sichtbarkeit
            icon.className = isPlaying ? 'fas fa-pause text-xl ml-0' : 'fas fa-play text-xl ml-1';
        }
    },

    skip: (seconds) => {
        if(Player.audio) Player.audio.currentTime += seconds;
    },
